...
> 4. In the url bar, type in the url like:
     e.g. http://www.example.com  http://bitcoin.org http://google.com http://columbia.edu
     e.g. http://code.activestate.com/recipes/408997-when-to-not-just-use-socketclose/
...
class Server:
    ...
    <html>
    <head>
    <h>This site can't be reached</h>
    </head>
    <body>
    <p>server IP address could not be found</p>
    <body>
    </html>
    """
    ...
    def read_request(self, conn, addr):
        ...
        try:
            ...
            # parser result
            print(self.getTimeStamp() + ' Request to Addr: {}   Port: {}'.format(webserver, port))
            #self.write_log('{}##{}'.format(webserver, port))
            self.write_log(self.getTimeStamp() + ' Request to Addr: {}   Port: {}'.format(webserver, port))


            try:
                socket.create_connection((webserver,port))
            except socket.error, err:
                print(self.getTimeStamp() + ' Bad Connection!'+str(err))
                self.write_log(self.getTimeStamp() + ' Bad Connection!'+str(err))
                page = self.page_bad_connection
                header = self.generate_header_lines(404, len(page))
    ...
    def check_cache(self, webserver, port,conn, header):
        ...
        # check if the IP address exist
        #try:
        #    socket.create_connection((webserver,port))
        #except socket.error.gaierror, err:
        #    print(self.getTimeStamp() + "IP doesn't exist")
        #    self.write_log(self.getTimeStamp() + "IP doesn't exist")
        #    page = self.page_bad_connection
        #    header = self.generate_header_lines(404, len(page))
        #    conn.sendall(header+page)
        #    conn.close()
        ...
        if not cache_status:
            ...
            #print(h)
            print(self.getTimeStamp() + 'Request the file from the remote server\nRequesting...\nRequest Message\n'+h)
            #self.write_log('Request-Server-From-Proxy\n'+h)
            self.write_log(self.getTimeStamp() + 'Request the file from the remote server\nRequesting...\nRequest Message\n'+h)
